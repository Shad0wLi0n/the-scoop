var yaml = require('write-yaml');//allows writing database to YAML file
const loadYamlFile = require('load-yaml-file');//allows loading database from YAML file
...
loadYamlFile('.database.yml').then(data => {//if the YAML database is populated it will restore the values
  if(data === undefined) {
    database = {
      users: {},
      articles: {},
...
function writeDatabaseToYamlDatabase() {
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const userCommentIds = database.users[user].commentIds;
